// -----------------------------------------------------
// Utility to replace the table Device in the database with 
// the content of devices.json. 
...
async function main (content) {
    ...
    try {
        ...
        if( devices.length > 0)
        { 
           var documentCount = await Device.countDocuments({});
           // erase the table :Member 
           await deleteAllData();

          console.log("Got a request to replace/restore the Device database out of a JSON file. "); // + JSON.stringify(request.body));  
          // iterate through the JSON records received but stored them as comma separated format (CVS)
          console.log( devices.length + " rows will replace "+ documentCount + " in the database." );

          await insertMany(devices);

         }
        // Count rows in the table
        documentCount = await Device.countDocuments({});
        console.log(documentCount +" records(s) were inserted to the Device's table.");
...
async function insertMany(devices) {
    try {
